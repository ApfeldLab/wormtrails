...
def write_mp4(video_array, out_path, fps=60):
    """
    Writes a Numpy array containing video frames to an MP4 file.
    
    Args:
        video_array: 3D or 4D Numpy array of 8 bit unsigned integers (uint8) containing the video frames. For 3D, time is axis 0 (monochrome). For 4D, shape is (time, height, width, channels) with 3 channels for color.
...
def write_avi(video_array, out_path, fps=60):
    """
    Writes a Numpy array containing video frames to an AVI file using FFV1 codec for lossless compression.
